...
function genTactDictGet(
    ...
    const dictGet = () => {
        ...
        return match(key)
            .on("slice")(() => ctx.used(`__tact_dict_get${cellSuffix}`))
            .on("uint")(() => `udict_get${cellSuffix}?`)
            .on("int")(() => `idict_get${cellSuffix}?`)
            .end();
    };
    const returnExpr = () =>
        match(value)
            .on("slice")(() => "r")
            .on("cell")(() => "r")
            .on("uint")(() => "r~load_uint(vl)")
            .on("int")(() => "r~load_int(vl)")
            .on("coins")(() => "r~load_coins()")
            .end();
    const valBitsArg = () =>
        match(value)
            .on("slice")(() => "")
...
function genTactDictExists(ctx: WriterContext, key: KeyType): void {
    ...
    const dictGet = () =>
        match(key)
            .on("slice")(() => ctx.used("__tact_dict_get"))
            .on("uint")(() => "udict_get?")
            .on("int")(() => "idict_get?")
            .end();
...
function genTactDictSet(
    ...
    const dictDel = () =>
        match(key)
            .on("slice")(() => ctx.used("__tact_dict_delete"))
            .on("uint")(() => "udict_delete?")
            .on("int")(() => "idict_delete?")
            .end();
    // prettier-ignore
...
function genTactDictGetMin(
    ...
    const returnValExpr = () =>
        match(value)
            .on("int")(() => "value~load_int(vl)")
            .on("uint")(() => "value~load_uint(vl)")
            .on("coins")(() => "value~load_coins()")
            .on("slice")(() => "value")
            .on("cell")(() => "value")
            .end();
...
function genTactDictGetNext(
    ...
    const dictGetNext = () =>
        match(key)
            .on("int")(() => "idict_get_next?")
            .on("uint")(() => "udict_get_next?")
            .on("slice")(() => ctx.used("__tact_dict_next"))
            .end();
    const returnValExpr = () =>
        match(value)
            .on("int")(() => "value~load_int(vl)")
...
function genTactDictReplaceGet(
    ...
    const dictDelGet = () => {
        const cellSuffix = value === "cell" ? "_ref" : "";
        return match(key)
            .on("slice")(() => ctx.used(`__tact_dict_delete_get${cellSuffix}`))
            .on("uint")(() => `udict_delete_get${cellSuffix}?`)
            .on("int")(() => `idict_delete_get${cellSuffix}?`)
            .end();
    };
    // prettier-ignore
    const returnExpr = () => match(key, value)
    ...
    const parseExpr = () =>
        match(value)
            .on("slice")(() => "old")
            .on("cell")(() => "old")
            .on("uint")(() => "old~load_uint(vl)")
            .on("int")(() => "old~load_int(vl)")
            .on("coins")(() => "old~load_coins()")
            .end();
